...
- **Portable dedicated-scroll build.** `MixCanvas-0.0.17-2026-08-03-DEDICATED-SCROLL-SURFACE-portable.exe` contains the `will-change: scroll-position` measurement. It launched successfully in software mode for six seconds. Size: 65,612,800 bytes; SHA-256: `B3D9F6F518601605F9027F0F839901FBA9AA5334AB7DA9053DC6959DF3DDB153`.

- **Dedicated scroll surface trial.** The Timeline viewport temporarily declared `will-change: scroll-position`. The paint-isolation trace showed one scrolling raster layer accounting for virtually all remaining graphics work; this targeted WebView2 hint was measured as the final no-UX-cost scroll optimisation.

- **Dedicated scroll surface rejected after measurement.** A 20.694-second trace measured 3.09 s/s of parallel RasterTask work versus 3.07 s/s in the 18.794-second paint-isolation trace: no material improvement. `will-change: scroll-position` is removed. The residual work is Chromium's already-dedicated software scroll layer, not React, layout, waveform construction or an unpromoted CSS surface.

- **Paint isolation validated.** Comparable ~19-second traces reduced Paint work from 1,090 calls / 828 ms to **434 calls / 449 ms**. Full-document paint peaks fell from 9.16 ms to 4.88 ms. The containment boundary is retained.

- **Portable paint-isolation build.** `MixCanvas-0.0.17-2026-08-03-TIMELINE-PAINT-ISOLATION-portable.exe` contains the isolated Timeline paint viewport. It launched successfully in software mode for six seconds. Size: 65,612,800 bytes; SHA-256: `2D67775FB807B2C685C13830751E54656CC4CE83F9D2E3B7464D34B38BC9D698`.

- **Timeline paint isolation.** `.timeline-scroll` is now a CSS layout-and-paint boundary. It is already a fixed clipped viewport, so this does not alter the visible Timeline; it prevents a scroll invalidation from needlessly propagating to the whole document in software rendering.

- **Portable local-seek build.** `MixCanvas-0.0.17-2026-08-03-TIMELINE-SEEK-LOCAL-COORDS-portable.exe` contains the corrected coordinate conversion for timeline clicks. It launched successfully in software mode for six seconds. Size: 65,612,800 bytes; SHA-256: `7E6682F16F2F2F98EC58818195D1D5B0FC8E437F88A8CDF79E9378E7FA0B964C`.

- **Timeline seek fixed for real.** The seek target is now calculated from the clicked lane/ruler's actual DOM rectangle. That rectangle already carries the current native scroll offset and virtual centring margin; subtracting the margin again forced normal clicks toward beat zero. A regression test covers seeking at the centred playhead of an overflowing timeline.
...
- **Le rendu logiciel devient un choix assumé, et non un dépannage.** Le comportement ne change pas — `RenderMode::Software` était déjà le défaut — mais la fenêtre `ABOUT` le présentait sous le titre « If the display flickers », comme si l'on contournait un problème d'affichage. La raison réelle est une mesure : sur les machines profilées, l'accélération matérielle n'a produit aucune différence sur le comportement de la timeline, tandis que certains pilotes font déchirer le compositeur de WebView2 au zoom. Entre aucun gain et un artefact possible, la justesse l'emporte.
  - Le titre devient « Why the interface draws in software », et le texte dit ce qui se passe pour l'audio : lecture, analyse, mixage et bounce sont du Rust natif et ne touchent jamais au moteur de rendu.
  - Les trois drapeaux sont documentés dans le `README` avec un tableau et un exemple, et le dossier `portable` reçoit le raccourci `--gpu-safe` qui manquait — la fenêtre `ABOUT` promet désormais un raccourci par mode, il fallait qu'ils existent tous les trois.
  - Réserve consignée : la comparaison qui exonère le GPU date du 31 juillet, quand le fil principal était encore saturé par nos propres défauts. Un résultat nul sous une ressource saturée ne prouve pas l'absence d'effet. Le drapeau reste donc à portée, et remesurer ne coûte qu'un lancement.

- **La porte de qualité repasse au vert, et elle cachait un second échec.** `check.cmd` sortait en code 1 sur `cargo fmt`, à quinze endroits dans quatre fichiers et non dans le seul `timeline.rs` qui était consigné. Comme le script s'arrête à la première étape en échec, **Clippy n'avait jamais tourné** : lancé à la main, il rejetait `draw_timeline_volume_shape` et `draw_timeline_pan_shape`, passées à neuf arguments.
  - Les deux commandes reçoivent six paramètres de domaine — sous la limite — plus les trois états gérés que Tauri injecte. Ce sont ces derniers qui font le compte, et `draw_timeline_filter_bubble` portait déjà l'exemption et son explication trois fonctions plus loin; les deux nouvelles avaient simplement été écrites sans la reprendre. Regrouper les paramètres ne ferait que masquer la signature IPC.
  - La leçon vaut d'être notée avant une version candidate : **tant que la porte est rouge, on ne sait pas ce qu'elle cache.** Le seul état exploitable est le code 0.

## 2026-08-02

- **Tempo ramps no longer re-render the timeline.** The live BPM readout now updates its text node directly. The stress trace showed 243 full layout passes in 16.7 seconds because a changing BPM lived in React state; the playhead, VU and BPM are now all outside that render loop.

- **Portable seek-fix build.** `MixCanvas-0.0.17-2026-08-03-SEEK-FIX-portable.exe` corrects timeline click seeking with centred native scrolling. It launched successfully in software mode for six seconds. Size: 65,612,800 bytes; SHA-256: `ED3E74E19CFA7AA2DA343B722A09F98A82AA5D5F7722E87739F8D336906AFE0C`.

- **Timeline click seeking restored.** A centred native-scroll timeline includes half a viewport of virtual side space; click coordinates now remove that space before conversion to musical beats. The regression is covered for both overflowing and fully fitted timelines.

- **Portable optimization build.** `MixCanvas-0.0.17-2026-08-03-CLIP-VIRTUALIZATION-portable.exe` packages the Draw-path and clip-virtualization optimizations with embedded resources. It launched successfully in software mode for six seconds. Size: 65,612,800 bytes; SHA-256: `2C4B03E5AB9F6D51CF164658BC6BDD552E5BDA3D9C40E54F69C63197EA5A6381`.

- **Timeline clip virtualization.** Off-screen clips no longer keep headings, buttons and waveform containers in the DOM. The rendered window includes a generous safety margin, while audio playback retains the complete timeline. Clip sequence numbers and library-track lookups are indexed instead of being recomputed by repeated array searches.

- **Draw automation completed.** Each Pan/Volume stroke persists its shape and period with the exact DSP samples, follows its clip, is restored by Undo/Redo and project files, and can be removed atomically with `Delete Draw` without affecting manual nodes.

- **A Draw now renders as one bounded visual stroke.** The detailed automation samples remain only for audio playback. The timeline creates one compact SVG path per Draw, scaled to the visible width; overlapping Pan and Volume Draws are both offered explicitly in the right-click menu.

- **Un Draw Pan/Volume est une courbe, pas une forêt de nodes.** La migration 26 ajoute `timeline_draw_groups` et rattache les points internes par `draw_group_id`. Ils restent exhaustifs pour le moteur audio, mais l'interface n'affiche que leur ligne d'automation; les nodes manuels restent visibles. Un clic droit dans la plage d'un Draw offre `Delete Draw` et supprime son groupe atomiquement, sans toucher aux nodes manuels.

- **Le suivi du timeline cesse de repeindre le timeline.** La trace sans halo VU ne contient plus aucune animation de `led-vu-segment`, mais elle nomme alors la vraie surface coûteuse : `timeline-content` est repeinte sur une zone non bornée, jusqu'à 15,7 ms, pendant que son `transform` et celui du playhead repartent chacun environ 18 fois par seconde. Le contenu reçoit une demi-fenêtre virtuelle de chaque côté et avance maintenant par écriture native de `scrollLeft`; le playhead devient une superposition fixe au centre du viewport.
  - Aucun `scrollLeft` n'est lu. L'ancienne lecture de vérification forçait déjà 188 ms/s de mise en page; la nouvelle mécanique ne fait qu'écrire la coordonnée dérivée du beat courant. Le centrage au début et à la fin du projet reste possible grâce aux deux marges statiques.
  - **Validation trace :** sur 23,896 s de lecture, `Layerize` passe de 194,8 à **11,6 ms/s** (−94 %), `Paint` de 178,0 à **93,7 ms/s** (−47 %) et `Layout` de 23,4 à **15,5 ms/s** (−34 %). Les pistes sont encore repeintes 20 fois/s en rendu logiciel, mais chaque passe de `timeline-tracks` plafonne à 3,19 ms et le conteneur complet n'est plus une surface de peinture non bornée.

- **Le portable est désormais construit par Tauri, pas par Cargo seul.** Un `cargo build --release` direct produisait un exécutable Rust valide mais sans le contexte de distribution Tauri : au lancement, il cherchait logiquement le serveur de développement `127.0.0.1:1420`. La distribution passe maintenant par `tauri build --no-bundle -f embed-resources`, qui construit Vite et embarque `frontendDist`; le binaire autonome ne dépend donc d'aucun serveur local.

- **La seconde trace confirme un gain, mais révèle que l'ombre des LED reste le mauvais levier graphique.** Les captures n'ont pas la même durée utile (16,737 s puis 19,675 s), donc leurs totaux bruts ne sont pas comparables. Rapporté à la seconde, `Layerize` passe de 223,4 à 200,1 ms/s et `Paint` de 177,1 à 155,6 ms/s : environ 10 à 12 % de moins. Pourtant Chromium décrit encore 384 animations `box-shadow` sur les lentilles VU de la seconde capture. Le halo des états actifs est donc retiré, pas seulement sa transition CSS; les LED conservent leurs couleurs et leur relief, mais aucune ombre ne change pendant la lecture.
  - La prochaine mesure pourra ainsi séparer ce coût de celui qui domine maintenant : les translations du contenu complet de timeline et du playhead, chacune déclenchée environ 18 fois par seconde pendant Play en mode logiciel.

- **La première trace de lecture isole le VU-mètre comme coût graphique concret.** Sur 18,272 s, le renderer passe 3 739 ms dans `Layerize` et 2 964 ms dans `Paint`, contre 361 ms dans les appels JavaScript de la page. La trace nomme 297 animations incapables d'être composées, toutes sur `box-shadow` de `led-vu-segment`; elle crée et détruit 1 267 / 1 238 calques. Les transitions de fond et d'ombre de ces LED sont retirées : la couleur et le halo de chaque état restent identiques, mais une diode affiche immédiatement la mesure courante au lieu d'animer une valeur déjà périmée. Ce correctif est délibérément isolé afin que la trace suivante mesure son gain avant toute intervention sur le déplacement de timeline.

- **Un cinquième du fil principal partait dans une lecture de `scrollLeft`.** Un `useLayoutEffect` sans dépendances vérifiait à chaque rendu que le décalage natif de la timeline valait bien zéro. Lire `scrollLeft` oblige le navigateur à recalculer style et mise en page **sur-le-champ**, avant de pouvoir répondre — et comme le rendu venait d'écrire des styles, il n'avait rien en réserve. Mesuré : 3 774 ms sur 20 s, soit 188 ms par seconde, pour vérifier une valeur qui ne bouge presque jamais.
  - `overflow` est déjà `hidden` des deux côtés : la molette ne peut pas faire défiler cet élément. Il ne reste qu'un défilement provoqué par le navigateur lui-même quand il met un enfant au premier plan — et l'évènement `scroll` dit exactement quand cela arrive. Un zoom remet le décalage à zéro par une **écriture seule**, qui n'interroge pas la mise en page.
  - C'est le profileur qui l'a nommée, en attribuant le *Recalculate style* à une position précise dans le paquet — `index-DnGaLsNR.js:9:129812`. Sans cette attribution, la lecture était invisible : elle ne ressemble pas à du travail.
  - Toutes les autres lectures qui forcent la mise en page ont été relevées d'un coup, la leçon des propriétés personnalisées étant retenue. Elles sont saines : `offsetHeight` n'est lu qu'au `pointerdown`, et les rectangles ne sont demandés que pendant un glisser. `scrollLeft` était la seule sur le chemin de chaque rendu.

- **Plus aucune propriété personnalisée n'est écrite depuis React.** Après en avoir corrigé deux occurrences une à une, elles ont été énumérées d'un coup — il en restait une troisième, et c'était la pire : `--anchor-offset`, posée **sur le clip**, donc sur l'ancêtre de sa waveform. Un seul descendant la lisait, un `span` de quelques pixels; elle invalidait le style calculé de tout le clip à chaque cran de zoom. Le décalage est maintenant écrit sur l'ancre elle-même, en `left` ordinaire.
  - `--seek-progress`, sur le curseur de lecture de l'aperçu, n'était lue par aucune règle. Supprimée, ainsi que les deux imports de type devenus morts avec elle.
  - La leçon vaut plus que les trois correctifs : chercher une occurrence à la fois a coûté trois journées de mesures là où une recherche exhaustive aurait tout donné du premier coup.

- **Le défaut du 31 juillet était encore là, un étage plus bas.** La grille de mesure des voies tirait sa période de `--measure-width`, une propriété personnalisée posée sur la voie elle-même — donc sur l'ancêtre de tous ses clips, de toutes ses waveforms et de tous ses nœuds. Changer une propriété personnalisée invalide le style calculé de tout le sous-arbre : à chaque cran de zoom, la voie entière était recalculée. Le même mécanisme que celui mesuré à 36 % à la racine, corrigé alors sans qu'on regarde en dessous.
  - La période passe par `background-size`, une propriété **ordinaire** : elle n'invalide que l'élément qui la porte. Le dégradé répétitif devient une tuile unique large d'une mesure, dont les quarts sont exprimés en pourcentages — la garantie d'alignement des quatre traits, qui était la raison de la forme précédente, tient de la même façon.
  - **`--beat-width` disparaît : aucune règle ne la lisait.** Elle était écrite sur deux éléments par voie, à chaque zoom, et invalidait leurs sous-arbres pour rien.
  - La couleur de fond de la bande de filtre sort du raccourci `background`, où elle formait une troisième couche sans image : les deux valeurs de `background-size` y étaient recyclées cycliquement pour la remplir. Deux couches déclarées, deux valeurs.

- **Les courbes d'automation ne sont plus construites que là où on les voit.** Même correction que pour les waveforms, et le même gaspillage : un chemin tracé d'un bout à l'autre du mix, dont on regarde un ou deux pour cent. Il coûtait trois fois — à la construction de la chaîne, à la mise en page du chemin, à sa peinture — et il était refait à chaque cran de zoom. Le découpage garde **un nœud de chaque côté** de la fenêtre, faute de quoi le segment qui entre dans le champ aurait une pente calculée depuis le bord et la ligne ne passerait plus où elle passe. Les extrémités restent ancrées à 0 et à la largeur totale : un segment plat de plus ne coûte rien et garde la courbe identique là où on la regarde.

- **Quarante-trois pour cent du fil principal partaient dans un curseur qui change de forme.** Le profil désignait `panNodeY` — trois opérations arithmétiques, incapables de coûter cela. V8 l'avait inlinée et le profileur lui créditait la fonction qui l'entoure : la **construction des courbes d'automation**. La vraie cause était en amont.
  - `updateSmartCursor` écrivait **deux objets neufs** dans l'état à chaque mouvement du pointeur. Le pointeur bouge plus de cent fois par seconde; l'outil, lui, ne change qu'en franchissant une frontière. Des objets neufs empêchent React de s'arrêter là : le panneau entier était re-rendu à la fréquence de la souris. Il ne s'écrit plus rien tant que rien ne change.
  - Et à chacun de ces rendus, les trois jeux de courbes — volume, panoramique, filtre — étaient reconstruits **d'un bout à l'autre du mix**, sur une timeline qui peut faire soixante-dix mille pixels de large. Ils ne dépendent que des nœuds, des brouillons en cours et du zoom : jamais du survol, jamais de la tête de lecture. Ils sont mémorisés.
  - Ce coût **précédait** les mesures des deux jours passés — il se cachait derrière `(anonymous)`, parce qu'un build minifié n'a plus de noms de fonctions. Il n'est apparu qu'une fois le bruit du style et de la peinture retiré, et une fois compilé un binaire de diagnostic non minifié. Deviner l'aurait manqué; trois relevés successifs l'ont trouvé.

- **Le transport sort du cycle de rendu de React.** Le correctif de la veille avait fait ce qu'il visait — *Recalculate style* de **36,4 % à 1,3 %**, *Layout* de 8,1 % à 1,1 %, trente fois moins de recalcul — mais le fil principal restait saturé : le travail s'était simplement déplacé. Nouveau relevé sur 9,71 s : **script 38,0 %**, Paint 20,5 %, Layerize 16,8 %. Le fil était si occupé à recalculer des styles qu'il *étranglait* React ; libéré, React rendait davantage.
  - La position de lecture est interrogée vingt fois par seconde. Tant qu'elle vivait dans un `useState`, chacune de ces réponses re-rendait l'application entière — pour déplacer deux éléments. Elle passe maintenant par `liveTransport`, un petit émetteur auquel on s'abonne : la translation du contenu, la tête de lecture et le curseur de la barre sont **écrits directement dans le DOM**, sans rendu.
  - Un rendu React n'est redemandé que lorsque la vue a glissé de **192 px** — le pas en deçà de la marge que les waveforms se gardent. C'est là qu'il y a vraiment de nouveaux marqueurs et de nouvelles tranches à construire. Aux réglages courants, cela passe d'un rendu toutes les 50 ms à un rendu toutes les quelques secondes.
  - **Le défilement horizontal en profite autant que la lecture.** Un geste de molette produit des dizaines d'évènements, et chacun re-rendait le panneau entier : c'est ce qui le rendait poisseux. Il ne fait plus qu'écrire trois propriétés.
  - Le VU-mètre s'abonne lui aussi, et ne se réveille que lorsqu'une **diode** change d'état — le niveau arrive en valeur continue, l'affichage n'a que vingt-quatre marches, et la plupart des mesures ne changeraient rien à l'écran.
  - Effet de bord bienvenu : une action déclenchée au clavier — couper au playhead, poser un nœud de volume — lit désormais la position **au moment du geste** et non celle du dernier rendu. Elle était juste par accident, parce que l'état se rafraîchissait vingt fois par seconde ; elle l'est maintenant par construction. Au passage, l'écouteur clavier global cesse d'être démonté et remonté vingt fois par seconde.

## 2026-07-31

- **Quarante-quatre pour cent du temps de rendu partaient dans deux lignes de CSS.** Mesuré sur la machine, inspecteur ouvert, cinq secondes de défilement avec deux pistes : le fil principal est occupé à **cent pour cent** — 8 368 ms de travail dans 8,37 s. Le détail par activité tranche : **Recalculate style 36,4 %**, Commit 14,9 %, script 11,8 %, Paint 10,0 %, **Layout 8,1 %**.
  - La cause : `--timeline-follow-offset`, une propriété personnalisée posée sur `.timeline-content` — l'ancêtre de **toute** la timeline — et changée vingt fois par seconde par le transport. Changer une propriété personnalisée invalide le style calculé de tout le sous-arbre : chaque clip, chaque waveform, chaque nœud, chaque marqueur était recalculé à chaque image. Et comme elle alimentait `left` et `margin-inline`, elle payait aussi la mise en page.
  - Remplacée par une **transformation**, écrite dans le rendu. Une transformation n'invalide pas le style des descendants et ne déclenche pas de mise en page.
  - Le commentaire d'origine justifiait `left` par un désaccord d'une image entre le playhead et la grille pendant un zoom. Ce défaut avait bien existé — mais il venait d'un mécanisme qui écrivait la transformation **hors** de React, en concurrence avec son rendu. Écrite *dans* le rendu, elle est publiée dans le même commit que la largeur et les coordonnées de tous les enfants : l'atomicité est exactement celle de `left`, sans le layout ni l'invalidation. Le choix d'alors traitait le symptôme d'un autre défaut.

- **Et le GPU est mis hors de cause, par la mesure.** Les deux modes donnent les mêmes chiffres à moins de deux pour cent près — 535 contre 541 ms par seconde de rendu. Le moteur n'était pas le problème, et remettre l'accélération n'aurait rien gagné. Trois jours de soupçons refermés par un relevé.

## 2026-07-30

- **Un portable qui cache sa base de données n'est pas portable.** Les médias avaient déménagé à côté de l'exécutable; la bibliothèque et les modèles déballés, eux, dormaient encore dans `%APPDATA%` — soixante-sept mégaoctets d'un côté, trente-cinq de l'autre, dans un dossier caché que personne ne pense à vider. On emportait le programme sans sa bibliothèque, et on ne savait pas quoi effacer pour repartir à neuf.
  - **Tout** ce que le programme écrit vit désormais dans `MixCanvas Files`, à côté de lui : la base, les ressources extraites de l'exécutable, les stems et les cuissons. On copie le programme et ce dossier, on a déménagé son installation entière; on efface le dossier, on repart d'une installation neuve.
  - La bibliothèque existante est **adoptée** au premier lancement, par le même mécanisme qui récupérait déjà celle des deux anciens noms du programme : rien à refaire, rien à réanalyser.
  - Le repli sur les données applicatives reste, pour un exécutable posé là où il n'a pas le droit d'écrire — refuser de démarrer serait pire. `ABOUT` le dit, et dit aussi l'exception.


- **La base rend au disque la place que les suppressions ont libérée.** SQLite réutilise ses pages libres mais ne rétrécit jamais le fichier de lui-même. Chaque réanalyse réécrit la grille de temps et la waveform du morceau, et chaque montée de version d'algorithme les réécrit toutes : le fichier monte à son plus haut niveau et y reste. Mesuré sur une bibliothèque de vingt-quatre morceaux — **soixante-sept mégaoctets pour dix de données vivantes**, cinquante-sept de vide.
  - Ce n'était pas une fuite : la place se réutilise, et le cache de waveform lui-même est borné — une ligne par morceau, emportée avec lui. Mais un fichier qui pèse sept fois ses données se copie et se sauvegarde sept fois trop lentement, et ressemble à un défaut.
  - Le compactage a lieu à l'ouverture, et seulement si le vide dépasse **à la fois** la moitié du fichier et seize mégaoctets : autrement on paierait une réécriture complète à chaque lancement. Un échec ne fait rien perdre et n'empêche pas de démarrer. Un test gonfle une base, la vide, vérifie qu'elle se compacte — puis qu'une base déjà saine n'est pas réécrite pour rien.


- **Deux clips étirés qui se superposent faisaient craquer la sortie. C'était ma régression du time-stretch.** En corrigeant celui-ci, j'avais rendu le rayon de recherche **toujours** maximal — juste, et nécessaire pour recaler la phase d'un grave — mais en notant chaque candidat à pleine résolution. Chiffré : cent trente-cinq mille interpolations par grain, contre six à quinze mille auparavant, soit **dix-huit à vingt-deux fois** le coût dans le cas le plus courant. Un clip passait; deux clips superposés demandaient 5,8 millions d'interpolations par seconde au fil audio.
  - La recherche devient **hiérarchique** : une passe à gros pas sur un point sur quatre trouve la bonne bosse, deux passes serrées trouvent son sommet. Le rayon ne change pas — c'est lui qui rend le recalage possible —, seul le nombre de candidats notés à pleine résolution baisse. **Huit fois moins de travail.**
  - **Aucune perte de qualité**, et c'est mesuré par l'instrument construit pour la correction d'origine : à 130 → 123 BPM, énergie hors raies à −16,83 dB — le chiffre exact d'avant —, aucun creusement d'enveloppe, et le même compte de frappes sur un train d'attaques.
  - **Un test compte désormais les interpolations d'un grain et les borne.** Rien ne mesurait ce coût, donc rien n'a signalé qu'il avait été multiplié par vingt. Le plafond tombera si quelqu'un rend la recherche exhaustive une seconde fois — et il vérifie aussi qu'elle cherche encore, un plafond tenu en ne faisant rien ne prouvant rien.


- **La waveform n'est plus construite que pour ce qu'on voit.** Mesuré avant de toucher à quoi que ce soit, sur un morceau de six minutes au zoom ordinaire : **seize mille colonnes**, un million six cent mille caractères de chemin, **vingt-six millisecondes par clip et par cran de zoom** — pour neuf cents pixels réellement à l'écran. Quatre-vingt-treize pour cent de ce travail ne se voyait jamais, et trois clips suffisaient à mettre quatre-vingts millisecondes dans chaque cran.
  - Le clip ne produit plus que la **tranche visible**, élargie d'une marge. Réduction mesurée du chemin : ×9 au zoom ordinaire, ×27 et ×55 en zoom serré; la construction passe de 26 ms à 1,4 ms. Un clip entièrement hors champ ne produit plus aucune géométrie.
  - **Le détail ne bouge pas.** Le niveau reste choisi sur la largeur entière du clip — c'est elle qui dit combien de colonnes il faut pour une colonne par pixel. Le viser sur la tranche donnait quatre-vingt-sept colonnes pour quatorze cents pixels : une waveform en escalier. Vérifié après correction : 0,75 pixel par colonne au zoom ordinaire, jamais plus d'une colonne par pixel manquante.
  - **La fenêtre glisse par pas de 256 pixels**, d'un bloc de largeur fixe. Sans ce pas on aurait troqué un gros calcul rare contre un petit calcul à chaque image de lecture; et avec deux bords alignés chacun pour soi, la tranche changeait deux fois par pas au lieu d'une. Un test mesure la **fréquence** des changements, pas leur date — la fenêtre s'aligne sur une grille fixe, donc elle bascule au passage d'une ligne.
  - Ce qui n'a pas été fait, et pourquoi : le passage au canvas 2D que recommande le document. Le gain de la réduction est certain et indépendant du moteur de rendu; celui du canvas dépend du coût de **peinture**, que je ne peux pas mesurer ici — le panneau de prévisualisation ne compose pas d'images, et un premier chronométrage m'a d'ailleurs donné un résultat trompeur. C'est une mesure à faire sur la machine, après celle-ci.
