...
Le même bouton engage une teinte de console en deux temps. D'abord un relief : plateau grave de +2 dB sous 90 Hz, puis **une cloche** de +1 dB centrée à 13 kHz, de facteur de qualité 1,2. Ensuite une saturation, car un relief ne fait que déplacer l'équilibre d'un signal — ce qui le colore vraiment, ce sont des harmoniques.

Le haut fut d'abord un plateau de +2 dB au-dessus de 10 kHz, et ce choix était mauvais pour une raison de forme : un plateau monte à sa pleine valeur et **y reste jusqu'à Nyquist**. Il levait donc 18 kHz aussi fort que la bande d'air — une étendue que rien ne reproduit et que personne n'entend, qui coûte de la marge et, sur le chemin MP3, coûte des bits. Une cloche fait le même travail là où il s'entend et revient à zéro au-dessus : +1,0 dB à 13 kHz, +0,13 à 18 kHz, +0,03 à 20 kHz.

Le grave, lui, a fait un aller-retour qui vaut d'être retenu comme leçon de méthode. En adoucissant le « smiling V » on l'a descendu de 2,0 à 1,5 dB **par symétrie**, alors que seul le haut posait problème. À l'écoute la coupe s'est révélée trop franche, et il est revenu à +2,0. Mesuré pondéré en bruit rose, l'étage de couleur retrouve alors exactement le +1,57 dB qu'il avait avant : le grave porte l'essentiel de cette énergie, si bien que le rendre suffit à ramener le niveau sans rien redonner à la partie du spectre qu'on voulait calmer. Corollaire : le seuil du limiteur de mastering, descendu à −4,0 dB pour compenser la perte, est revenu à −3,7 puisque la perte n'existe plus — un défaut doit rester rattachable à quelque chose, et celui-là ne l'était plus qu'à un défaut réparé.

Ce bouton est **allumé d'entrée** dans une bibliothèque neuve. Un bus de console a sa colle et sa couleur en permanence; les découvrir par un interrupteur éteint veut dire mixer un moment sans, puis tout rééquilibrer en les allumant. Une bibliothèque existante garde ce sur quoi elle a été laissée, et un projet enregistré porte son propre état : un changement de défaut ne doit pas réécrire le son de ce qui est déjà fait.
...
Le mode de rendu se choisit **au lancement**, et non plus à la compilation. La variante `disable-gpu` a disparu : `RenderMode` lit `--no-gpu`, `--gpu-safe` ou `--gpu` et ajuste `WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS` avant la création du premier WebView, en préservant les arguments déjà présents. Le dessin logiciel a été le défaut de juillet 2026 à la 1.6, parce que certains pilotes faisaient déchirer le compositeur matériel de WebView2 pendant un zoom et qu'aucun soin de notre côté n'y changeait rien. Une seule feature de compilation subsiste, `embed-resources`, qui produit le portable d'un seul fichier.

Premier artefact observé en rendu matériel depuis ce basculement : la fenêtre d'égalisation d'un clip clignotait le temps d'une trame, sur une carte NVIDIA d'une génération précédente, et seulement en accélération complète — `--no-gpu` et `--gpu-safe` en étaient tous deux exempts. La cause n'était pas le mode de rendu mais un `backdrop-filter` : c'était le seul de la feuille posé sur du contenu en mouvement, puisque l'éditeur de grille et l'aide se consultent à l'arrêt tandis que l'EQ reste ouvert pendant que la timeline défile. Le compositeur devait relire le fond à chaque image. Le flou est retiré et le voile densifié; le défaut ne remet pas le défaut de rendu en question, mais il rappelle où regarder en premier la prochaine fois — vers ce qu'on demande au compositeur, avant de soupçonner le pilote.

**Le défaut est l'accélération complète depuis la 1.6.** Le compromis a changé de sens, et c'est la correction de la géométrie de zoom qui l'a retourné : une fois les éléments synchronisés à chaque cran, ce qui restait était une image blanche pendant que le processeur repeignait seul une surface longue. Le déchirement qui avait motivé le repli logiciel n'est pas reparu sur la session lourde de référence, alors que le rendu matériel supprime visiblement l'image vide. La mesure prime sur le principe, mais elle a été prise sur une machine : `--no-gpu` reste la porte de sortie, et `--gpu-safe` l'entre-deux, précisément parce que ce type de défaut appartient au pilote plus qu'au programme.

La découpe d'un clip s'arrête au temps entier le plus proche. La tête de lecture est à la milliseconde près et couper là donnait des clips de 4,0173 temps; or une ancre est un entier et `snap_anchor_beat` la pose à la mesure, si bien qu'aucune position de la grille ne pouvait suivre un tel clip bout à bout. Boucler une mesure prise à la main laissait donc un trou de silence, que rien ne permettait de combler — la seule place autorisée mordait sur l'originale de dix-sept millièmes de temps. L'arrondi vit dans `split_timeline_clip` et non dans l'interface : la touche `B` et le menu doivent couper au même endroit, et une règle posée à un seul des deux finit par diverger.

Le format de projet doit porter **tout** ce qu'une session contient, et c'est le contrat le plus facile à rompre du programme. Une propriété nouvelle traverse sept endroits — schéma SQLite, instantané Rust, types TypeScript, format de fichier, restauration d'Undo, duplication, scission — et l'ajouter à cinq d'entre eux suffit à faire passer tous les tests. C'est exactement ce qui est arrivé deux fois : les quatre champs de coupure et de boucle, puis les quatre collections de passes d'effets, ont vécu dans la base et le moteur sans jamais atteindre le fichier. Les tests comptaient les clips et vérifiaient les positions, jamais le reste. Un aller-retour compare désormais **chaque** champ réglé loin de sa valeur d'usine, et un second vérifie qu'un fichier ancien, privé de ces champs, se relit encore.

Le chargement d'un projet **vide les tables d'effets sans condition**, y compris pour un fichier qui n'en porte aucune. Sans cette purge, ouvrir un projet laissait jouer les passes de la session précédente : les collections étaient indépendantes des clips et rien ne les effaçait. Le défaut se cachait derrière l'usage le plus courant — rouvrir aussitôt dans la même base, où les passes en place sont justement les bonnes.

Un clip supprimé laisse ses médias en attente. `clip_stems` et `clip_bakes` tombent par cascade avec lui, et l'instantané d'annulation ne décrit que ce qui se dessine : il ne peut donc pas les reconstruire. `remove_clip` les verse d'abord dans `removed_clip_stems` et `removed_clip_bakes` — schéma 37, waveforms comprises, car un relévé de chemins rendrait le son sans le dessin — et `restore_snapshot` les reprend. Le rattachement tient au fait qu'un clip restauré conserve **son identifiant d'origine**; le `clip_id` de ces tables n'a donc pas de clé étrangère, puisqu'il désigne précisément un clip qui n'existe plus. Le cycle de vie se règle à la fermeture : l'historique vit dans l'interface et meurt avec la fenêtre, donc les tables sont vidées là, juste avant le balayage qui récupère la place. Ce balayage compte ces tables parmi les références, sans quoi il effacerait le fichier que l'annulation s'apprête à rendre.

Le décuissonnage n'efface plus rien. Vérifier que plus aucun clip de la session ne désigne le fichier paraît suffisant et ne l'est pas : un `.mixcanvas` enregistré plus tôt peut encore le référencer, et le scénario tient en quatre gestes — cuire, enregistrer, décuire, rouvrir la sauvegarde. Prouver qu'un chemin est dans notre dossier ne prouve pas que plus personne n'en a besoin. La place se récupère par le balayage de fermeture, qui ne visite que `Scratch` : les médias d'un projet nommé lui sont invisibles par construction.

Les médias fabriqués appartiennent à des **fichiers**, pas à des lignes. Une scission et une duplication recopient le chemin d'un stem ou d'une cuisson, pas son contenu : deux clips désignent alors le même WAV. Deux défauts symétriques en découlaient. Au déménagement, `relocate_project_media` traitait les lignes une à une, si bien que la première portait le fichier et que la seconde, sa source ayant disparu, gardait un chemin vide — la première sauvegarde cassait la moitié des références. À l'inverse, `unbake_clip` rendait le chemin à effacer sans regarder qui d'autre le désignait. Le déménagement porte maintenant chaque fichier une seule fois puis réécrit toutes ses références, et le décuissonnage ne rend un fichier que lorsque plus rien ne le désigne. Une seconde garde s'y ajoute, `is_inside_media_root` : le chemin d'une cuisson traverse le fichier de projet, qui est du JSON modifiable à la main, et rien d'autre n'empêcherait un projet trafiqué de faire effacer un fichier quelconque.

L'écriture d'un projet se fait à côté puis **écrase la destination par renommage**, sans la retirer d'abord. Une première version le faisait, au motif que Windows refuserait un renommage sur un fichier existant : c'est faux, `rename` s'appuie sur `MoveFileEx` avec remplacement. La suppression était donc inutile, et c'est elle qui ouvrait le trou qu'on voulait fermer — passé ce point, la dernière version valide n'existait plus nulle part, et un échec du renommage ne la rendait pas. Le temporaire porte aussi un nom unique créé en exclusivité : le dériver de la destination par `with_extension` faisait qu'enregistrer sous un nom déjà terminé par l'extension de travail visait le même fichier des deux côtés, et l'opération effaçait sa propre source. `fs::write` tronque avant de remplir, et cet intervalle coûtait la dernière version valide en cas de coupure ou de disque plein — celle-là même qu'on remplaçait. Le `sync_all` avant le renommage n'est pas décoratif : sans lui, le nom peut devenir visible avant les octets, et une coupure laisserait un fichier au bon nom et au contenu tronqué, ce qui est pire qu'un ancien perdu puisqu'il a l'air bon. Le temporaire est voisin de la destination, un renommage ne traversant pas les volumes.

La bibliothèque se filtre par une loupe posée au bout de la rangée de tri. `src/lib/librarySearch.ts` tient la correspondance, séparée de l'affichage pour être vérifiable seule : les deux textes sont ramenés en minuscules, privés de leurs accents et de leur ponctuation, puis chaque mot tapé doit se retrouver quelque part — pas forcément dans l'ordre ni côte à côte. « kalk 2005 » trouve donc `01-paul_kalkbrenner-gebrannt-2005.mp3`, et « gebrannt » trouve *Gebrännt*. La recherche porte sur l'artiste, le titre, le nom de fichier et le chemin complet, ce dernier parce qu'on se souvient parfois du dossier plutôt que du morceau. L'album n'y est pas : la trame `TALB` n'est pas lue et aucune colonne ne la conserve, si bien que l'ajouter serait un chantier d'étiquettes et non de recherche. Le tri s'applique avant le filtre, l'ordre dépendant de la place d'un morceau dans la timeline et non de ce qu'on cherche. Rien n'est retenu d'une session à l'autre : rouvrir le programme sur une bibliothèque amputée sans savoir pourquoi serait le pire des services.

Le Clip EQ porte **deux** cloches paramétriques et non une. Elles sont identiques et appliquées l'une après l'autre; deux cloches en série additionnent leurs courbes, donc l'ordre entre elles ne s'entend pas et n'est fixé que pour que le dessin et le son désignent la même. L'ajout n'a demandé **aucune migration** : `eq_settings` est du JSON dans une colonne TEXT, si bien qu'un projet antérieur relit une seconde cloche absente comme une cloche neutre.

Le graphe de cette fenêtre ne montre que les filtres. Il incluait le gain du clip dans la courbe tout en plaçant la poignée de la cloche sans lui : deux références pour un même dessin. Sur un clip remonté de cinq décibels, la courbe partait donc à +5, plafonnait au sommet du cadre dès que la cloche dépassait +1 et se dessinait plate, pendant que la poignée continuait de monter — on croyait l'égaliseur à bout de course alors que rien ne le bornait. La capacité n'était pas en cause : gain et cloche sont limités séparément, +12 et +6 dB, et le moteur les additionne. Un gain est un niveau, pas un filtre; le cadre garde sa pleine amplitude quel que soit le réglage de sortie.

Les réglages de la boîte de bounce — limiteur et format — sont écrits **aux deux sorties de la boîte**, et non au seul lancement d'un rendu. Ce sont des réglages de chaîne, ajustés bien avant qu'on exporte quoi que ce soit; ne les enregistrer qu'en bouçant faisait perdre au redémarrage tout ce qu'on avait posé puis annulé.

La reprise d'une bibliothèque héritée ne s'applique **que** dans les données applicatives. `adopt_legacy_library` existe pour une installation qui a survécu au changement de nom du programme — Tauri dérive le dossier de données de l'identifiant, donc renommer l'application le fait pointer ailleurs. Sa seule garde était « pas de base à l'arrivée », ce que tout dossier portable neuf remplit : la migration ne s'exécutait pas une fois mais à chaque copie de l'exécutable, et y ressuscitait une bibliothèque qu'on croyait avoir laissée derrière. Copier le programme dans un dossier vide doit donner une bibliothèque vide; c'est la promesse du portable, et c'était exactement ce que cette routine ne tenait pas. Aucune version publiée n'a jamais écrit sa base dans les données applicatives — `media_root` est antérieur à la 1.0.0 — donc seul un poste ayant fait tourner une build de développement pouvait être touché.

La moitié basse d'une enveloppe de volume suit une loi de fader et non une règle graduée. Elle répartissait quarante décibels également sur la course : régulier à l'œil, faux à l'oreille. À mi-course on était à − 20 dB, soit un dixième de l'amplitude là où l'on attend « moitié moins fort »; un dixième de course coûtait déjà quatre décibels, donc la retouche fine sous l'unité était impossible; et le dernier quart courait de − 30 à − 40 dB, une zone inaudible dans un mix. La position est maintenant élevée au carré (`VOLUME_TAPER`), ce qui place la mi-course à − 10 dB — la valeur d'un fader normalisé — et rend la résolution dix fois plus fine près de l'unité. C'est un changement de **dessin** seulement : le moteur reçoit toujours des décibels et un projet enregistré sonne à l'identique, ses nœuds étant simplement redessinés plus haut. Effet de bord assumé : sous l'unité la courbe est plus fine que le dixième de décibel affiché, donc les deux premiers pour cent de la course lisent 0,0 dB — un cran d'unité de deux pixels, qui permet d'y revenir sans viser.

Le sidechain distingue trois rôles, et non deux. La clé faisait plonger **tout** ce qu'elle recouvrait, si bien que la configuration la plus utile à trois pistes — la source, un clip qui pompe, un troisième intact — était inexprimable. Lequel épargner relève du mix et rien ne permet de le deviner : `SidechainRole` porte donc `None`, `Key` et `Ducked`, et la touche de chaîne les parcourt dans cet ordre. La clé reste exclusive quand les receveurs ne le sont pas, et un clip ne peut pas être les deux — la source s'entendrait pomper elle-même. Le schéma 38 ajoute `ducks_under_key`, neutre pour un clip neuf; la migration met les clips existants à 1, un changement de règle ne devant pas réécrire le son de ce qui est déjà fait. Un projet antérieur, où le champ est absent, se relit de même avec tous ses clips receveurs.

Un clip cuit peut être séparé, et c'est **son fichier cuit** qu'on sépare. La cuisson passait avant tout le reste dans le choix de la source audio, sans condition : sur un clip cuit, `VOX` allumait sa touche sans rien changer à ce qu'on entend, et lançait au passage une séparation de plusieurs minutes dont le résultat n'était jamais lu. Séparer la source n'aurait rien réparé — cette voix-là n'a pas les effets qu'on vient de cuire. Le fichier cuit, lui, les porte, et sa voix est le même son une voix plus loin. Les deux jeux coexistent donc, distingués par `clip_stems.from_bake` : un clip cuit joue le stem tiré de sa cuisson, un clip nu celui tiré de son morceau, et le graphe suit la même règle. Décuire retire les stems de la cuisson — ils décrivent un fichier qu'on ne joue plus — et laisse intacts ceux du morceau; si la source n'a pas la voix que le clip jouait, le clip revient au morceau entier plutôt que de garder une touche allumée qui ne décrit plus rien. Les fichiers séparés portent des noms distincts selon leur origine, sans quoi le second effacerait le premier sur le disque.

Un clip peut être **bouclable**, par sa touche `∞`. C'est un état et non une action : tant qu'il dure, les deux poignées cessent de rogner et allongent la boucle de part et d'autre du motif, lequel reste décrit par le rognage. Éteindre la boucle rend donc le clip exactement tel qu'il était, et remet les débordements à zéro — garder une longueur qu'on ne voit plus serait une mémoire cachée dont on ne se méfie qu'une fois.

La répétition n'a pas lieu à l'intérieur d'une voix. `loop_tiles` déplie la boucle en clips de rendu consécutifs à la construction du plan : le moteur ne voit que ce qu'il sait déjà jouer, et le son est celui de copies posées à la main. Boucler dans la voix aurait demandé de recoudre le fenêtrage WSOLA au point de bouclage, à chaque tour. Les pavés sont calés pour qu'un d'eux tombe exactement sur le motif d'origine, sans quoi allonger d'un côté déplacerait ce qu'on entend de l'autre; ceux des extrémités sont partiels, ce qui n'est qu'un rognage de plus. `loopTurns`, côté interface, est le miroir exact de `loop_tiles` et doit le rester : ce qu'on voit et ce qu'on entend sont deux lectures du même découpage, et deux carrelages qui divergent donnent une waveform qui ment.

Un clip peut être coupé seul, par sa touche `M`. C'est une décision de mix qui appartient au clip, là où le `MUTE` de la voie répond à une autre question et emporte les clips voisins. Un clip coupé n'entre pas dans le plan de rendu — ni décodeur ouvert, ni cycle dépensé pour du silence, et rien dans le bounce — mais **compte toujours dans `end_beat`**. C'est la condition pour que la carte de tempo du transport et celle du rendu continuent de concorder : `project_timing` compte tous les clips, et un plan qui s'arrêterait plus tôt ferait perdre au transport, silencieusement, le suivi des éditions en cours de lecture.

`duplicate_clip` essaie quatre places et prend la première libre : la voie du dessous aux mêmes temps, celle du dessus, puis collé à droite et collé à gauche sur la même voie. L'ordre vient d'un principe unique — ne rien déplacer. Une copie sur une voie voisine ne consomme aucune place sur l'axe du temps; les deux autres en consomment, donc elles passent après. Le pas latéral est l'étendue du clip **rognée**, arrondie à la mesure supérieure : les ancres sont toutes des multiples de `BEATS_PER_MEASURE`, et une copie posée hors grille ne pourrait plus être ramenée à sa place par un glissement. On ne cherche pas au-delà de l'accolement, parce qu'une copie qui part se poser hors de l'écran n'est pas ce qu'on a demandé en cliquant sur ce clip-là; l'échec est un message. `duplicate_placements` est isolée de la base pour que cette arithmétique de mesures se teste seule.

La copie emporte ce qui appartient au clip — égalisation, rognage, voix choisie, stems séparés, cuisson — et rien de ce qui appartient à la voie. Volume, panoramique, filtres et passages d'effets ont été tracés à un endroit d'une piste, pas sur un clip : les recopier écrirait par-dessus ce qui est déjà là où la copie atterrit. Une exception mérite d'être nommée : le `removed` d'une cuisson est réécrit à vide sur la copie. Il ne sert qu'à rendre à la voie l'automation que la cuisson lui a prise, et la copie n'en a pris aucune; le recopier tel quel ferait qu'un `UNBAKE` sur la copie effacerait l'automation de l'originale pour la réécrire ailleurs.

Le décodage MP3 passe par `rodio::Decoder`, donc par `symphonia-bundle-mp3`. Une sonde comparative (`symphonia_agrees_with_a_reference_decoder`, en `#[ignore]` dans `audio/bounce.rs`) mesure ce chemin contre ffmpeg, décodeur écrit indépendamment, sur un même MP3 CBR 320. Résultat sur un matériel poussé à −3,1 dBFS : **décalage de zéro trame** — le retard d'encodeur inscrit par LAME est honoré à l'identique, donc la grille de temps ne glisse pas à l'import — et un écart RMS de 5,4 × 10⁻⁷, soit 1,8 % d'un pas de quantification seize bits. Les deux décodeurs ne divergent jamais au-delà d'une décision d'arrondi. La question était posée après qu'un décodeur externe eut faussement montré une perte dans le grave; elle est réglée par la mesure, et la sonde reste pour pouvoir la reposer.

La même mesure a mis au jour un rabotage. `symphonia-bundle-mp3` 0.5.5 termine son banc de filtres de synthèse par un `clamp(-1.0, 1.0)`, alors qu'un décodage MP3 dépasse la pleine échelle : le codec ne reconstruit pas les crêtes exactement. Ces dépassements sont donc écrêtés à l'import. L'ampleur mesurée est de dix échantillons sur 705 600 — 0,0014 % — sur un fichier volontairement limité à −0,01 dBFS, et ce qui disparaît est de l'énergie fabriquée par le codec, absente de l'enregistrement d'origine. C'est pourquoi rien n'est fait : Symphonia 0.6 a retiré ce `clamp`, mais y basculer le décodage demanderait un adaptateur pour la source de lecture de rodio et une passe complète sur la chaîne audio, pour récupérer un dépassement d'artefact que le limiteur reprendrait de toute façon.

Un détail à connaître avant de rouvrir ce dossier : **Symphonia 0.6 est déjà lié dans l'exécutable, et n'y sert à rien.** `beat-this` le déclare en dépendance non optionnelle avec `features = ["all"]`, donc tous ses codecs sont compilés; mais `analysis.rs` décode par `crate::audio::open_mp3_decoder`, c'est-à-dire par rodio, et ne passe à `BeatThis` que des échantillons déjà décodés. Le lecteur de `beat-this` n'est jamais appelé. C'est heureux : deux décodeurs en service auraient pu traiter le retard d'encodeur différemment, et la grille de temps aurait glissé par rapport au son qu'elle décrit. Un seul décodeur touche les fichiers, et c'est celui de rodio.
...
npx tauri build --no-bundle -f embed-resources --config src-tauri/tauri.diagnostic.conf.json
```

`src-tauri/tauri.diagnostic.conf.json` ne fait que quatre lignes et n'est versionné que pour cette recette : il vide `beforeBuildCommand`, faute de quoi Tauri refabrique un frontend minifié par-dessus celui qu'on vient de produire lisible, et le profileur retombe sur `(anonymous)`.
...
Le moteur applique le filtre après le time-stretch, avant l'automation de volume et avant la sommation master. Ce jalon utilise un biquad Butterworth à Q fixe (`0,707`) : la valeur positive pilote un passe-haut de 50 Hz à 12 kHz, la valeur négative un passe-bas de 18 kHz à 90 Hz. Ces bornes évitent des extrêmes peu musicaux. Un mix sec/filtré maintient le bypass exact; un gain de compensation est appliqué ensuite. Le passe-haut suit une rampe linéaire en dB de 0 à +4,5 : ce qu'il retire s'entend partir, et ce qui reste est aigu, donc léger en crête. Le passe-bas suivait la même rampe jusqu'à +6 dB. La mesure la dément. En sonie pondérée K, fermer le passe-bas jusqu'à 260 Hz sur un morceau de dance ne coûte que 0,4 dB — le grave portait déjà l'essentiel de l'énergie — pendant que le rattrapage ajoutait 4,8 dB de crête. Ces décibels n'apportaient rien à l'oreille et poussaient un grave déjà à pleine échelle contre le limiteur, qui sur une onde à 55 Hz suit le cycle et déforme au lieu de retenir. Le passe-bas suit donc une bosse, `2,0 × sin²(π·course) + 0,8 × course` : un sommet de 2,4 dB à mi-balayage, là où le retrait du médium s'entend, puis un retour à 0,8 dB à la fermeture, où il ne reste qu'un grave que l'oreille compte à peine et que la crête compte pleinement. La valeur audio est lissée sur 8 ms pour éviter un pop lors d'une automation abrupte ou d'un passage de LP à HP. La tension du node de départ d'un segment est persistante et s'édite avec `Shift + molette` sur ce segment. React échantonne la courbe de puissance et Rust applique exactement la même fonction : `progress ^ (2 ^ (tension × 2))`. La courbe dessinée est donc celle entendue, sans segments droits d'automation.
...
- sa longueur;
- ses réglages d'égalisation propres (**Clip EQ** : passe-haut, deux cloches paramétriques, passe-bas, gain) et ses fenêtres de découpage non destructif (**subclips**);
...
Lorsqu'un clip est déplacé :
  ...
  - `src/components/ClipEqModal.tsx` : Fenêtre modale d'Égaliseur de morceau (Clip EQ 4 bandes + Trim Gain).
...
Une rangée de bibliothèque ne porte plus de flèche d'écoute. Cliquer le nom lance la même preview, et c'est le geste qu'on tente en premier devant une liste de musique : la flèche doublait donc ce geste dans une colonne à elle. Elle coûtait une trentaine de pixels, sa largeur et son espace de colonne, sur les trois cents que la rangée accorde au titre — un dixième du nom lisible échangé contre un raccourci vers ce que le nom faisait déjà. La grille de la rangée n'a plus qu'une colonne. Elle était déclarée trois fois, dont une à cinq colonnes : seule la dernière déclaration s'appliquait, les deux autres décrivaient une table qui avait eu des colonnes BPM et Actions. Le BPM n'y est plus une colonne depuis longtemps, il se lit sur la ligne du titre.
